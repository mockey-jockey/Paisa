...
const Dashboard = (props) => {
  ...
  const RenderMonths = () => {
    if(lastSixMonths.length){
      return <Text style={{display: "flex",justifyContent:'space-evenly'}}>{lastSixMonths.map((item,index) => <Button primary text={item} key={index} style={{text:{textAlign:"center",alignSelf:"stretch"}}} />)}</Text>
    }
    return <Text></Text>;
  }

  ...
  const renderItem = ({ item }) => (
    <View style={card().cardViewStyle}>
      <View style={card().leftElements}>
        ...
        <Text style={{}}>{item.date} {item.mode === 'credited' ?  <Icon.Button
        name="arrowup"
        backgroundColor="#3b5998"
      ></Icon.Button> : item.mode === 'debited' ?  <Icon.Button
      name="arrowdown"
      backgroundColor="#3b5998"
    ></Icon.Button> : null}</Text>
      </View>
      <View style={card().rightElements}>
        <Text style={{}}>{item.amount} <Text>{item.type}</Text></Text>

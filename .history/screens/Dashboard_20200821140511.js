...
const Dashboard = (props) => {
  ...
  const getLastSixMonthsData = async () => {
    try {
      ...
      getTransactions(new Date(2020,7,20).setHours(0,0,0,0),new Date().getTime());
    } catch(error) {
      console.log(`******${error}*******`);
    }
  }
  ...
  const _retrieveData = async (minDate,maxDate) => {
    try {
        ...
        if (transactions !== null) {
          var minDate = new Date(2020,7,20).setHours(0,0,0,0);
          var maxDate = new Date().getTime();
          let parsedTransactions = JSON.parse(transactions);
          parsedTransactions.filter((item) => {
            console.log(item.timestamp,minDate,maxDate)
            console.log(item.timestamp > minDate)
            console.log(item.timestamp > maxDate)

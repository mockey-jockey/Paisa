...
class SendSMSContainer extends Component {
    ...
    isYesterday = (date) => {
        ...
        const yesterday = new Date(msgDate.setDate(msgDate.getDate()-1));
        ...
        console.log(`***${yearDate}*${yearNow}**${monthDate}*${monthNow}****${dayDate}*${dayNow}**`);
    ...
    getAcctAndCardNumber = (message) => {
        var acctRegex = /(?:\sa\/c\s|\sacct\s*)([0-9|*|X]{6})/gmi,
    ...
    getMerchantName = (message) => {
        ...
        //var regex = /(?:\sat\s|\sin\s|\sto\s*)([A-Za-z0-9|+|@|\/]*\s?-?\s?[A-Za-z0-9|+|@|\/|-]*\s?-?\.?)/gmi;
        var atRegex = /(?:\sat\s|\sin\s)(.*\s)(on)/gmi,merchantName;
        if(atRegex && atRegex.length){
            merchantName = regex.exec(message);
        }

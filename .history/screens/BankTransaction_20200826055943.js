...
const BankTransaction = (props) => {
  ...
  const RenderBankTransactions = ({type}) => {
    ...
    if(banks.length){
      return (<View style={{flex:1,}}>{banks.map((item,index) => {
        ...
        return (<Card key={index} style={{padding:0}} >
  ...
  const RenderMonths = () => {
    if(props.route.params.months.length){
      return <View style={{display:'flex',flexDirection:'row',justifyContent:'space-around'}}>{props.route.params.months.map((item,index) => <Text primary testID={item} key={index} style={{display:'flex',justifyContent:'center',padding:10}} onPress={() => getTransactionsDate(this, item)}>{item}</Text>)}</View>
    }
    return <Text></Text>;
  }
  // <Picker

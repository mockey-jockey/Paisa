  ...
  const RenderBankTransactions = () => {
    ...
    if(banks.length){
      return (<View>{banks.map((item,index) => {
        ...
        const balance = transactionDetails[item].expanse.map((item) => item.availBalance);
        return (<Card onPress={viewTransactions}>
            <View style={{display:'flex',flexDirection:'row',justifyContent:'space-around'}}>
              <Text testID={item} key={index}>{item}</Text>
              <Text testID={item} key={`${index} balance`}>{balance}</Text>
            </View>
            <Divider/>
            <FlatList data={transactions} renderItem={renderItem} keyExtractor={item => item.index}/>
          </Card>)

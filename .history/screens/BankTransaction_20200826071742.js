  ...
  const RenderBankTransactions = ({type}) => {
    ...
    if(banks.length){
      return (<View style={{flex:1}}>{banks.map((item,index) => {
        ...
        return (<View key={index} style={{display:'flex',flexDirection:'column',backgroundColor:'#fff'}}>
    ...
    return <View style={{flex:1,display:'flex',flexDirection:'column',backgroundColor:'#fff',justifyContent:'center',alignItems:'center'}}><Text style={{fontSize:20,color:'#ccc'}}>No Transactions</Text></View>;
  }

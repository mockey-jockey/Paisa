        ...
        try{
            SmsAndroid.list(
                ...
                (count, smsList) => {
                    ...
                    var bankNames =  ["VYSA", "ALLA", "ANDB", "BARB", "BKID", "CBIN", "CITI", "CORP", "ICICI", "IDIB", "IOBA", "KVBL", "MAHB", "PUNB", "SBIN", "UBIN", "VIJB", "YESB", "BKDN", "CNRB", "DBSS", "IBKL", "IDFB", "INDB", "SYNB", "TMBL", "UCBA", "UTIB"];
    
                    arr.forEach((object,index) => {
                        //console.log('Object: ' + object);
                        var bankNameIndex = bankNames.filter((item) => object.address.indexOf(item));
                        console.log(bankNameIndex,object.address);
                        if(bankNameIndex !== -1){
                            ...
                            if(object.body.indexOf('withdrawn') !== -1){
                                bankList[name].push(parseMessage(object,'withdrawn',index.toString(),name));
                            }else if(object.body.indexOf('debited') !== -1){
                                bankList[name].push(parseMessage(object,'debited',index.toString(),name));
                            }else if(object.body.indexOf('spent') !== -1){
                                bankList[name].push(parseMessage(object,'debited',index.toString(),name));
                            }else if(object.body.indexOf('deposited') !== -1){
                                bankList[name].push(parseMessage(object,'credited',index.toString(),name));

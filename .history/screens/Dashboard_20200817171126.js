...
const Dashboard = (props) => {
  ...
  const getLastSixMonths = () => {
    ...
    console.log(month.map((item) => item.substr(0,3)).reverse())
    setLastSixMonths(month.map((item) => item.substr(0,3)).reverse());
  }
  ...
  const RenderMonths = () => {
    if(lastSixMonths.length){
      return <Text style={{display: "flex",justifyContent:'space-evenly'}}>{lastSixMonths.map((item,index) => <Button primary text={item} key={index} style={{container:{alignSelf:"stretch"},text:{textAlign:"center",alignSelf:"stretch"}}} />)}</Text>
    }
    return <Text></Text>;
  }

...
const Dashboard = (props) => {
  ...
  const RenderMonths = () => {
    if(lastSixMonths.length){
      return <ScrollView  horizontal={true} style={{display:'flex',flexDirection:'row',padding:0,margin:0}}>{lastSixMonths.map((item,index) => <Button primary text={item} key={index} style={{text:{display:'flex',borderColor:'red',borderWidth:1,fontFamily: 'lucida grande'}}} />)}</ScrollView>
    }
    return <Text></Text>;
  }

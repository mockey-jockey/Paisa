...
const Dashboard = (props) => {
  ...
  const RenderMonths = () => {
    if(lastSixMonths.length){
      return <View style={{display:'flex',flexDirection:'row',justifyContent:'space-around'}}>{lastSixMonths.map((item,index) => <Text primary text={item} key={index} style={{display:'flex',borderColor:'red',borderWidth:1,justifyContent:'center',borderRightColor:'#cccccc'}}>{item}</Text>)}</View>
    }
    return <Text></Text>;
  }

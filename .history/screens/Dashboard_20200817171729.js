...
const Dashboard = (props) => {
  ...
  const RenderMonths = () => {
    if(lastSixMonths.length){
      return <Text style={{display: "flex"}}>{lastSixMonths.map((item,index) => <Button primary text={item} key={index} style={{text:{textAlign:"center",fontFamily: 'lucida grande',}}} />)}</Text>
    }
    return <Text></Text>;
  }

...
const UPIRef = (message) => {
    var regex = /(?:(?:UPI).*?)(\d+(\d+)?)/gi;
    var upi = regex.exec(message);
    return upi && upi.length && upi[1] || '';
}
                ...
                (count, smsList) => {
                    ...
                    arr.forEach((object,index) => {
                        ...
                        if(isBankNameMatch.length){
                            ...
                            var upi = UPIRef(object.message);
                            if(upi && object[index+1] && object[index+1].message.indexOf(upi) !== -1){

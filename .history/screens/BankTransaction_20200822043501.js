...
const BankTransaction = (props) => {
  ...
  const RenderDates = () => {
    if(dates.length){
      return <View style={{display:'flex',flexDirection:'row',justifyContent:'space-around'}}>{dates.map((item,index) => <Text primary testID={item} key={index} style={{display:'flex',justifyContent:'center',padding:10}} onPress={() => getTransactionsDate(this, item)}>{item}</Text>)}</View>
    }
    return <Text></Text>;
  }
  return (
    <View>
        ...
        >
          {props.route.params.months.map((item,index) => <Picker.Item testID={index} key={index} label={item} value={item} />)}
        </Picker>
        <View>
             <RenderDates />
        </View>

    ...
    if(banks.length){
      return (<View>{banks.map((item,index) => {
        ...
        return (<Card key={index}>
            ...
            <FlatList data={transactions} renderItem={renderItem} keyExtractor={item => item.index}/>
            <Divider/>
            <View style={{display:'flex',justifyContent:'flex-end',padding:10}}>
              <Text style={{display:'flex',flexDirection:'row',justifyContent:'flex-end',padding:10}} onPress={() => viewTransactions(this,item)} testID={item} key={`${index} balance`}>View Transactions <Icon name="angle-double-right" size={20} color="#000" /></Text>
            </View>
          </Card>)
      })
      }</View>)

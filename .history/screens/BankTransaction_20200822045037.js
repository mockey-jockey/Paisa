...
const BankTransaction = (props) => {
  ...
  const RenderDates = () => {
    if(dates.length){
      return <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}  style={{display:'flex',flexDirection:'row',justifyContent:'space-around'}}>{dates.map((item,index) => <Text primary key={index} style={{display:'flex',justifyContent:'center',padding:10}} onPress={() => getTransactionsDate(this, item)}>{item}</Text>)}</ScrollView>
    }
    return <Text></Text>;
  }
  return (

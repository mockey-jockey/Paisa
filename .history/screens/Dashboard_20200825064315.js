...
const Dashboard = (props) => {
  ...
  const RenderBankTransactions = () => {
    ...
    if(banks.length){
      return <View style={{flex:1}}>{banks.map((item,index) => <Card><Text testID={item} key={index}>{item}</Text><Divider/><FlatList data={transactionDetails.transactions} renderItem={renderItem} keyExtractor={item => item.index}/></Card>)}</View>
    }
    return <Text></Text>;
  }

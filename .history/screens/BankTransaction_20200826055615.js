...
const BankTransaction = (props) => {
  ...
  const RenderMonths = () => {
    if(props.route.params.months.length){
      return <View style={{display:'flex',flexDirection:'row',justifyContent:'space-around'}}>{props.route.params.months.map((item,index) => <Text primary testID={item} key={index} style={{display:'flex',justifyContent:'center',padding:10}} onPress={() => setSelectedValue(this, item)}>{item}</Text>)}</View>
    }
    return <Text></Text>;
  }
  // <Picker

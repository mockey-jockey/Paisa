...
const Dashboard = (props) => {
  ...
  const RenderBankTransactions = () => {
    ...
    if(banks.length){
      return <View>{banks.map((item,index) => <Card onPress={viewTransactions}><Text testID={item} key={index}>{item}</Text><Divider/><FlatList data={transactionDetails[item].transactions.slice(0,5)} renderItem={renderItem} keyExtractor={item => item.index}/></Card>)}</View>
    }
    return <Text></Text>;
  }

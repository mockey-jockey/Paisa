...
const BankTransaction = (props) => {
  ...
  const RenderBankTransactions = () => {
    ...
    if(banks.length){
      return (<View>{banks.map((item,index) => {
        const transactions = transactionDetails[item].transactions;
  ...
  return (
    <View>
        ...
        <BottomNavigation active={active} children={<Text>today</Text>} hidden={true} >

...
const BankTransaction = (props) => {
  ...
  const RenderBankTransactions = ({type}) => {
    ...
    if(banks.length){
      return (<View style={{flexGrow:1}}>{banks.map((item,index) => {
        ...
        return (<Card key={index} style={{padding:0,margin:0,display:'flex'}} >
            <View style={{display:'flex',flexDirection:'row',justifyContent:'space-between',padding:10}}>
              <Text testID={item} key={index}>{item}</Text>
              <Text testID={item} key={`${index} balance`}>{balance.length && balance[0]}</Text>
            </View>
            <Divider/>
            <ScrollView style={{ flex: 1, flexGrow: 1 }} contentContainerStyle={{ flex: 1,flexGrow: 1 }}>

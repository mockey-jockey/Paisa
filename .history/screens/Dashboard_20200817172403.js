...
const Dashboard = (props) => {
  ...
  const RenderMonths = () => {
    if(lastSixMonths.length){
      return <View style={{display:'flex',borderColor:'green',borderWidth:1}}>{lastSixMonths.map((item,index) => <Button primary text={item} key={index} style={{text:{display:'flex',borderColor:'red',borderWidth:1,fontFamily: 'lucida grande'}}} />)}</View>
    }
    return <Text></Text>;
  }

    ...
    if(banks.length){
      return (<View>{banks.map((item,index) => {
        ...
        return (<Card key={index}>
            ...
            <FlatList data={transactions} renderItem={renderItem} keyExtractor={item => item.index}/>
            <Divider/>
            <Text style={{display:'flex',flexDirection:'row',alignItems:'flex-start',padding:10}} onPress={() => viewTransactions(this,item)} testID={item} key={`${index} balance`}>View Transactions <Icon name="angle-double-right" size={20} color="#fff" /></Text>
          </Card>)
      })
      }</View>)
    }

...
const Dashboard = (props) => {
  ...
  const getTransactionsDate = (_this,month) => {
    console.log("******",_this.testID,month)
    if(month === 'Today'){
      var start = new Date();
      start.setHours(0,0,0,0);
      getTransactions(start.getTime(),new Date().getTime());
  ...
  const RenderMonths = () => {
    if(lastSixMonths.length){
      return <View style={{display:'flex',flexDirection:'row',justifyContent:'space-around'}}>{lastSixMonths.map((item,index) => <Text primary testID={item} key={index} style={{display:'flex',justifyContent:'center',padding:10}} onPress={(this,item) => getTransactionsDate(this,item)}>{item}</Text>)}</View>
    }
    return <Text></Text>;
  }

...
const Dashboard = (props) => {
  ...
  const getLastSixMonthExpanse = (transactions) => {
      ...
      for(var i=0;i<lastSixMonths.length;i++){
        console.group(lastSixMonths[i])
      }
      // let expanseAmount = transactions.filter((item) => {
      //   let bankName = item.accountDetails.bankName;
      //   if(bankName === 'HDFC' && (item.type === 'debited'|| item.type === 'withdrawn') && item.timestamp > minDate && item.timestamp < maxDate){
      //     return item.amount
      //   }
      // });
      // expanse.push(expanseAmount.reduce((a,b) => a+b));
      console.log(expanse);
  ...
  const RenderBankTransactions = () => {
    ...
    if(banks.length){
      return (<View>{banks.map((item,index) => {
        ...
        return (<Card onPress={() => viewTransactions(this,item)}>
  ...
  return (
    <View style={styles.scrollViewStyle}>
        <StatusBarView lastSixMonths={lastSixMonths} lastSixMonthsExpanse={lastSixMonthsExpanse}/>
        <SafeAreaView>
          <HeaderView />
          <View>
             <RenderMonths />

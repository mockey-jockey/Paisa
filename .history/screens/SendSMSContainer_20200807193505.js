...
class SendSMSContainer extends Component {
    ...
    getMerchantName = (message) => {
        ...
        if(merchantRegex && merchantRegex.length){
            ...
            merchantName = `A/C ${acct[0]}` || '';
        }else if(card && card.length){
            merchantName = `Card ${acct[0]}` || '';
        }
    ...
    parseMessage = (object,type) => {
        console.log('------------------------------------------->');
        var merchantName = this.getMerchantName(object.body);
        var number = JSON.stringify(this.getAcctAndCardNumber(object.body,object.address))
        console.log('-->' + object.body);
        console.log('-->' + merchantName ? merchantName : number.accountNumber);

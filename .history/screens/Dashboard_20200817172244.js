...
const Dashboard = (props) => {
  ...
  const RenderMonths = () => {
    if(lastSixMonths.length){
      return <Text style={{display:'flex',borderColor:'red',borderWidth:1,fontFamily: 'lucida grande'}}>{lastSixMonths.map((item,index) => <Button primary text={item} key={index} style={{text:{}}} />)}</Text>
    }
    return <Text></Text>;
  }

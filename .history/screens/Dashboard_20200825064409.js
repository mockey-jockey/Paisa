...
const Dashboard = (props) => {
  ...
  const RenderBankTransactions = () => {
    ...
    if(banks.length){
      return <View>{banks.map((item,index) => <Card style={{height:'200px'}}><Text testID={item} key={index}>{item}</Text><Divider/><FlatList data={transactionDetails.transactions} renderItem={renderItem} keyExtractor={item => item.index}/></Card>)}</View>
    }
    return <Text></Text>;
  }
  ...
  return (
    <View style={styles.scrollViewStyle}>
        ...
        <SafeAreaView>
          ...
          <Button default text="Create post" onPress={viewTransactions}/>
